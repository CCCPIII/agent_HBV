...
def _backfill_history(session, period: str = "1mo") -> None:
    ...
    try:
        ...
        print("  [WARN] yfinance not installed, skipping historical backfill")
        return
    ...
    for ticker in tickers:
        try:
            ...
            print(f"  [WARN] backfill error for {ticker}: {exc}")

    print(f"  [OK] Historical backfill: {saved_snaps} price snapshots, {saved_news} news items")
